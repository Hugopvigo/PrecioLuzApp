...
import es.hugopvigo.precioluz.ui.theme.AuroraBlueDark
import es.hugopvigo.precioluz.ui.theme.AuroraBlueLight
import es.hugopvigo.precioluz.ui.theme.AuroraGreenLight
import es.hugopvigo.precioluz.ui.theme.AuroraPinkDark
import es.hugopvigo.precioluz.ui.theme.AuroraPinkLight
import es.hugopvigo.precioluz.ui.theme.AuroraPurpleDark
import es.hugopvigo.precioluz.ui.theme.AuroraTealDark
import es.hugopvigo.precioluz.ui.theme.AuroraYellowLight
...
private fun androidx.compose.ui.graphics.drawscope.DrawScope.drawDarkAurora(
    ...
    val blobs = listOf(
        Triple(Offset(w * 0.14f + dx, h * 0.10f + dy), AuroraPurpleDark.copy(alpha = .5f), w * .62f),
        Triple(Offset(w * 0.88f + dx, h * 0.06f + dy), AuroraBlueDark.copy(alpha = .48f),  w * .65f),
        Triple(Offset(w * 0.86f + dx, h * 0.84f + dy), AuroraTealDark.copy(alpha = .42f),  w * .75f),
        Triple(Offset(w * 0.06f + dx, h * 0.90f + dy), AuroraPinkDark.copy(alpha = .42f),  w * .70f),
    )
    blobs.forEach { (center, color, radius) ->
        drawCircle(
            brush  = Brush.radialGradient(
